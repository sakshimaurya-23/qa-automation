...
def generate_test_case(tc, baseline_dir, output_dir):
    ...
    # Gap fix: APIs whose <Input> templates reference ${ShipmentNo_Extracted}
    # (i.e. require a shipment to already exist) but which never themselves
    # produce a ShipmentNo. Without a prior createShipment call, the
    # placeholder is never resolved and keywords.robot silently SKIPS the
    # file at runtime (see Fix 9/unresolved-placeholder guard) — the step
    # never executes and nobody notices because the suite still reports PASS.
    SHIPMENT_DEPENDENT_APIS = {"shipDepart", "confirmShipment"}
    SHIPMENT_PRODUCER_API = "createShipment"

    def _inject_step(step_api, label):
        """Generate the Input/(validate)/Expected files for one auto-injected API,
        mirroring the normal per-step generation logic below. Returns updated counters."""
        nonlocal input_idx, expected_idx
        ...
        if not api_config.get("skip_input", False) and input_file:
            ...
            step_mappings.append({"step": label, "api": step_api, "mapped": True})
            print(f"  📄 Input: {target.name} ({step_api}) [auto-injected prerequisite]")
        ...
        if validate_file:
            ...
            print(f"  📄 Input: {vt.name} (validation) [auto-injected prerequisite]")
            ...
            if expected_name:
                ...
                print(f"  📄 Expected: {et.name} [auto-injected prerequisite]")
    ...
    for step in steps:
        ...
        # Auto-inject createShipment immediately before the first shipment-dependent
        # step if nothing earlier in the sequence already produced a ShipmentNo.
        if step_api in SHIPMENT_DEPENDENT_APIS and SHIPMENT_PRODUCER_API not in used_apis:
            if SHIPMENT_PRODUCER_API in API_STEP_MAP:
                print(f"  🔄 '{step_api}' requires ShipmentNo but no createShipment step found "
                      f"— auto-injecting createShipment before it")
                _inject_step(SHIPMENT_PRODUCER_API,
                              f"Auto-injected: createShipment (prerequisite for {step_api})")
            else:
                print(f"  ⚠ '{step_api}' requires ShipmentNo but createShipment baseline is not "
                      f"configured in API_STEP_MAP — this step will be SKIPPED at runtime "
                      f"(unresolved ${{ShipmentNo_Extracted}}). Add a createShipment step to "
                      f"the CSV or extend API_STEP_MAP.")
